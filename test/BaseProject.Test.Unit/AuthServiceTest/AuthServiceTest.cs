...
namespace AssetManagement.Test.Unit.AuthServiceTest
{
    public class AuthServiceTest
    {
        ...
        public async Task LoginAsync_ValidCredentials_ReturnsTokenAndUserResponse()
        {
            ...
            userRepositoryMock.Setup(repo => repo.GetAsync(u => !u.IsDeleted && u.Username == username, u => u.Role, u => u.Location))
                .ReturnsAsync(user);
            ...
            unitOfWorkMock.Setup(uow => uow.TokenRepository).Returns(tokenRepositoryMock.Object);
        ...
        public async Task RefreshTokenAsync_ValidRefreshToken_ReturnsNewTokenAndUserResponse()
        {
            ...
            var userResponse = new GetUserResponse();
            var tokenRepositoryMock = new Mock<ITokenRepository>();
            _unitOfWorkMock.Setup(uow => uow.TokenRepository).Returns(tokenRepositoryMock.Object);
        ...
        public async Task ResetPasswordAsync_ValidUserName_ReturnsNumberOfChanges()
        {
            ...
            var tokens = new List<RefreshToken> { new RefreshToken { TokenHash = "otherToken" } };
            ...
            _unitOfWorkMock.Setup(r => r.RefreshTokenRepository.GetAllAsync(It.IsAny<Expression<Func<RefreshToken, bool>>>()))
                .ReturnsAsync(tokens);
            // Act
            var result = await _authService.ResetPasswordAsync(userName, newPassword, refreshToken);
        ...
        public async Task ResetPasswordAsync_InvalidUserName_ThrowsKeyNotFoundException()
        {
            ...
            // Act & Assert
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _authService.ResetPasswordAsync(userName, newPassword, refreshToken));
        }
        ...
        public async Task ChangePasswordAsync_ValidCredentials_ReturnsNumberOfChanges()
        {
            ...
            var numberOfChanges = 1;
            var refreshToken = "validRefreshToken";
            var currentToken = "validCurrentToken";
            var refreshTokens = new List<RefreshToken> { new RefreshToken { TokenHash = "otherToken" } };
            var tokens = new Token {  HashToken = "otherToken"  };

            _unitOfWorkMock.Setup(uow => uow.UserRepository.GetAsync(It.IsAny<System.Linq.Expressions.Expression<Func<User, bool>>>()))
                .ReturnsAsync(user);
            _unitOfWorkMock.Setup(uow => uow.TokenRepository.GetAsync(x => x.HashToken != refreshToken))
                .ReturnsAsync(tokens);
            ...
            // Act
            var result = await _authService.ChangePasswordAsync(userName, oldPassword, newPassword, refreshToken, currentToken);
        ...
        public async Task ChangePasswordAsync_InvalidUserName_ThrowsKeyNotFoundException()
        {
            ...
            // Act & Assert
            await Assert.ThrowsAsync<KeyNotFoundException>(() => _authService.ChangePasswordAsync(userName, oldPassword, newPassword, refreshToken, currentToken));
        }
        ...
        public async Task ChangePasswordAsync_IncorrectOldPassword_ThrowsUnauthorizedAccessException()
        {
            ...
            // Act & Assert
            await Assert.ThrowsAsync<UnauthorizedAccessException>(() => _authService.ChangePasswordAsync(userName, oldPassword, newPassword, refreshToken, currentToken));
        }
