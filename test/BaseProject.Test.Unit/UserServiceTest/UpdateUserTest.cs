    ...
    public class UpdateUserTest
    {
        ...
        public async Task UpdateUserAsync_PassAllConstraint_UpdateToDatabaseAndReturnsUpdateUserResponse()
        {
            ...
            _unitOfWorkMock.Setup(x => x.UserRepository.GetAsync(a => a.IsDeleted == false && a.Id == userId, x => x.Role))
                .ReturnsAsync(user);
            _unitOfWorkMock.Setup(x => x.TokenRepository.GetAllAsync(a => a.UserId == userId))
                .ReturnsAsync(tokens);
            _unitOfWorkMock.Setup(x => x.BlackListTokenRepository.AddAsync(It.IsAny<BlackListToken>()))
                .Returns(Task.CompletedTask);
            _unitOfWorkMock.Setup(x => x.RefreshTokenRepository.GetAllAsync(a => a.UserId == userId))
            ...
            // Act
            var result = await _userService.UpdateUserAsync(userId, editUserRequest, Guid.NewGuid());
            // Assert
        ...
        [Fact]
        public async Task UpdateUserAsync_SelfEdit_ReturnsUpdateUserResponse()
        {
            // Arrange
            var userId = Guid.NewGuid();
            var roleId = Guid.NewGuid();
            var expectedMessage = "Can't edit yourself";
            var role = new Role
            {
                Id = roleId,
                Name = RoleConstant.STAFF
            ...
            // Act
            var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await _userService.UpdateUserAsync(userId, editUserRequest, userId));
        ...
        public async Task UpdateUserAsync_RoleNotFound_ThrowArgumentExceptionWithMessage()
        {
            ...
            // Act
            var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await _userService.UpdateUserAsync(userId, editUserRequest, Guid.NewGuid()));
        ...
        public async Task UpdateUserAsync_UserNotFound_ThrowArgumentExceptionWithMessage()
        {
            ...
            // Act
            var exception = await Assert.ThrowsAsync<ArgumentException>(async () => await _userService.UpdateUserAsync(userId, It.IsAny<EditUserRequest>(), Guid.NewGuid()));
